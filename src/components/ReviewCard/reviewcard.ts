...
class ReviewCard extends HTMLElement {
    ...
    attributeChangedCallback(propName:Attribute2,oldValue: string | undefined,newValue: string | undefined){
        switch(propName){
            case Attribute2.rating: //id transform to number to string. All the same in those propieties with number
    ...
    connectedCallback(){
      ...
      // Formatear la fecha para que se muestre en días, horas, minutos o segundos.
    formatTimeAgo(dateadded: any) {
        ...
        return `hace ${Math.floor(secondsElapsed / 86400)}d`;
    }
    ...
    async render() {
        if (this.shadowRoot) {
            ...
            <div class="card-container">
                <div>
                    <img src="${this.imageprofile || 'No image'}" alt="${this.user || 'No user'}">
                    <p>${this.user || 'No user'}</p>
                    <p>${this.bio || 'No bio'}</p>
                </div>
                <div>
                    <img src="${this.imagecover || 'No image'}" alt="Image cover">
                ...
                <div id="iconCard">
                    <svg class="heart-icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="gray" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314"/></svg>
                </div>
            </div>
            `;

            // Funcionalidad para cambiar el color del corazón
            const iconCard = this.shadowRoot?.querySelector('#iconCard svg');
            iconCard?.addEventListener('click', () => {
                const isActive = iconCard.classList.toggle('active');
                iconCard.setAttribute('fill', isActive ? '#4C248A' : 'gray');
            });
        }
    }
}
